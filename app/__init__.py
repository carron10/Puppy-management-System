...
def set_notifications(limit=None):
    ...
    for puppy in puppies:
        missing_updates_message, missing_dates = check_missing_updates(
            puppy.id)
...
def get_puppies():
    ...
    if id:
        parent = PuppyAdult.query.filter_by(
            id=id, user_id=current_user.id).first()
        if parent and parent.puppies:
            for puppy in parent.puppies:
                puppy_dict = puppy.to_dict(
                    rules=['-parent.puppies', '-records', '-recommendations', '-records', '-parent.user'])
...
def update_puppy_health():
    ...
    try:
        ...
        return "Invalid date format", 400
    if _date < puppy.birth_date:
        return "Date should be more than birth date"

    if _date < puppy.birth_date:
        return "Date should be later than birth date", 400
    # Check if a record for the given date already exists
    existing_record = db.session.query(PuppyRecords).filter_by(
        puppy_id=puppy_id, date=_date).first()

    if existing_record:
        return "Record for this date already exists", 400

    # Create a new record
    record = PuppyRecords(date=_date, puppy_id=puppy_id,
    ...
    if recommendation is not None:
        return {
            ...
            "status": recommendation.health_status,
            "tag": recommendation.tag,
            "id": recommendation.id,
            "puppy_id": recommendation.puppy_id
        }, 201
    return {}
