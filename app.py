...
from models import Role, User, db,Puppy,PuppyAdult,build_sample_db,PuppyRecords
from utils import get_puppies_for_user,get_puppy_by_id
...
def get_puppies():
    ...
    if id:
        ...
        if parent and parent.puppies:
            data.extend([puppy.to_dict(rules=['-parent.puppies','-records','-parent.user']) for puppy in parent.puppies])
    else:
        parents = PuppyAdult.query.filter_by(user_id=current_user.id).all()
        for parent in parents:
            if parent.puppies:
                data.extend([puppy.to_dict(rules=['-parent.puppies','-weights','-parent.user']) for puppy in parent.puppies])
    return data
